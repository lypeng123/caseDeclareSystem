...
public class UserManageController {
     ...
     * @return
     */
    @RequestMapping("/returnWord")
    public JsonResult<Object> download(HttpServletRequest request, HttpServletResponse response) {
        String ipAddr = IPvalidateUtil.getIpAddr(request).split(",")[0];// 访问的ip
        String old_file_name = request.getParameter("old_file_name");
        String emailAddr = request.getParameter("user_name");
        //删除本地文件
        JsonResult<Object> jsonResult = userManageService.deletePlanFile(response, request);
        if (jsonResult.getState() != 1) {
            //删除失败
            return new JsonResult<>(JsonResult.ERROR, "本地文件删除失败");
        }
        logger.info("IP为：【" + ipAddr + "】退回了案例" + old_file_name);
        //向用户邮箱发送退回邮件提醒
        SendEmailUtils.sendEmailConfigure(emailAddr, emailAddr, emailAddr, "");
        return new JsonResult<>(JsonResult.SUCCESS, "本地文件删除成功");
    }
}
