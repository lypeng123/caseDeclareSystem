...
$(function () {
    ...
    getAllProvince();
    caseScreenResult(pageNum, file_type, area_id, first_score);
    ...
    //查询案例筛选
    function caseScreenResult(pageNum, file_type, area_id, first_score) {
        $.ajax({
            type: 'post',
            url: caseScreenResultUrl,
            data: { audit_state: 1, pageNum: pageNum, file_type: file_type, area_id: area_id, average: "", first_score: first_score },
            xhrFields: {
                withCredentials: true
            },
            crossDomain: true,
            success: function (res) {
                ...
                if (res.state == 2) {
                    ...
                    if (res.data == null) {
                        ...
                        getfileHtml = ' <thead><th>文件号</th> <th>申报公司</th> <th><span class="sele-all">全选</span>申报文件</th> <th>第一轮专家评审得分</th> <th>第一轮专家评审人数</th> <th>提交时间</th> <th >操作</th></thead><tbody>';
                        ...
                        for (var i = 0; i < list.length; i++) {
                            ...
                            getfileHtml += ' <tr ' + setDataString(list[i]) + '> <td>' + list[i].file_code + '</td> <td>' + list[i].company_name + '</td> <td><span class="sele-icon no-icon" data-code="' + list[i].file_code + '" data-id="' + list[i].user_id
                                + '" data-score="' + firstScore + '"></span>' + list[i].old_file_name + '</td>  <td>' + firstScore + '</td> <td>' + first_count + '</td><td>' + uploadTime + '</td> <td><span class="preview_file" data-file=' + list[i].file_name + ' data-user_id="' + list[i].user_id + '" data-html_path="' + list[i].html_path + '">查看</span></td> </tr>';
                        }
                        getfileHtml += '</tbody>';
    ...
    var pass_file = parent.document.getElementById("pass_file2");
    var pass_list = parent.document.getElementById("pass_list2");
    var nopass = parent.document.getElementById("nopass2");
    ...
    //确定通过
    $(pass_file).on("click", "button#pass_con2", function () {
        $(pass_file).css("display", "none");
        $(pass_list).css("display", "block");
        var audit_state2 = 2;
        editCaseScreen2(audit_state2);
    });

    //不通过
    $(nopass).on("click", "button#pass_con2", function () {
        $(pass_file).css("display", "none");
        $(pass_list).css("display", "block");
    });

    //不通过发送信息
    $(nopass).on("click", "button#nopass_con2", function () {
        var audit_state2 = -1;
        editCaseScreen2(audit_state2);
        caseScreenResult(pageNum, file_type, area_id, first_score);
        $(nopass).hide();
    ...
    //查看清单
    $(pass_list).on("click", "button#view_list2", function () {
    ...
    function caseScreenResult2(pageNum, area_id2, file_type2, first_score2) {
        $.ajax({
            type: 'post',
            url: caseScreenResultUrl,
            data: { audit_state: 2, pageNum: pageNum, file_type: file_type2, area_id: area_id2, average: "", first_score: first_score2 },
            xhrFields: {
                withCredentials: true
            },
            crossDomain: true,
            success: function (res) {
                ...
                if (res.data == null) {
                    ...
                    var list = res.data.pageBeanMysql.data;
                    getfileHtml = '<thead> <tr> <th>文件号</th> <th>申报公司</th> <th>申报文件</th> <th>第一轮专家评审得分</th> <th>提交时间</th> <th >入选时间</th> <th>操作</th> </tr> </thead> <tbody>';
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        getfileHtml += '  <tr ' + setDataString(list[i]) + '> <td>' + list[i].file_code + '</td> <td>' + list[i].company_name + '</td> <td>' + list[i].old_file_name + '</td> <td>' + firstScore + '</td> <td>' + uploadTime + '</td> <td>' + selectedTime + '</td> <td><span class="preview_file" data-file=' + list[i].file_name + ' data-user_id="' + list[i].user_id + '" data-html_path="' + list[i].html_path + '">查看</span></td> </tr>';
                    }
                    getfileHtml += '</tbody>';
    ...
    //通过-不通过文件函数
    function editCaseScreen2(audit_state2) {
        var screen_param = {
            file_codes: [],
            user_ids: [],
        };
        ...
        if (audit_state2 == 2) {//复筛通过
            state = '3';
        } else if (audit_state2 == -1) {//复筛不通过
            state = '4';
        }
    ...
    $(pass_file).on("click", "#pass_can2", function () {
        $(pass_file).css("display", "none");
    });
    ...
    $(nopass).on("click", "#nopass_can2", function () {
        $(nopass).css("display", "none");
    });
