    ...
    function renderCaseList(data) {
        ...
        if (list.length == 0) { //无符合结果
            ...
            for (var i = 0; i < list.length; i++) {
                ...
                tbodyInnerHtml += '<tr' + setDataString(item) + '><td>' + item.user_name + '</td><td id="userId" style="display:none">' + item.user_id + '</td><td>' + item.password + '</td><td>' + item.type + '</td><td>' + item.state + '</td><td>' + item.company_name + '</td><td>' + item.responsible_person + '</td><td><a href="javascript:;" onclick="modify(' + item.user_id + ')" class="preview ' + previewSwitch + '">修改</a><span class="split">|</span><a href="" class="download">删除</a><span class="split">|</span><a href="javascript:;" id="checkCase">案例</a></td></tr>';
            };
